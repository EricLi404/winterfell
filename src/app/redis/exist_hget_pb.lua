...
local code_msg = app_config.code_msg()
local pb = require("pb")
local redis = require("lualib.redis_util")

ngx.req.read_body()
local request_byte = ngx.req.get_body_data()
pb.loadfile(config.pd_file_path)

local request = pb.decode("Request", request_byte)
...
local function response_msg(msg)
    ...
    local encodeStr = pb.encode('Response', response_table)
